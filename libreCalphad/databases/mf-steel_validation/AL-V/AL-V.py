...
comps = ['AL', 'V', 'VA']
conditions = {v.X('V'): (0, 1, 0.01), v.T: (300, 2500, 20), v.P: 101325, v.N: 1}
binary = BinaryStrategy(db, comps, phases, conditions)
binary.initialize()
binary.do_map()
plot_binary(binary, ax=ax)
fig.tight_layout()
plt.title(f"{comps[0]}-{comps[1]} Phase Diagram, {date.today()}")
plt.savefig(f"./{comps[0]}-{comps[1]}-phase_diagram.png")
ax.set_xlim((0, 0.025))
plt.savefig(f"./{comps[0]}-{comps[1]}_Al-rich_corner.png")