...
for temp in temps:
    ...
    conds = {v.T: temp, v.P: 101325, v.X('FE'): (0, 1, 0.01), v.X('S'): (0, 1, 0.01)}
    ternary = TernaryStrategy(db, comps, phases, conds)
    ternary.initialize()
    ternary.do_map()
    plot_ternary(strategy=ternary, x=v.X('TI'), y=v.X('S'), label_nodes=True, ax=ax)
    plt.title(f"{system} system, {temp} K, {date.today()}, Hirata 2017")
    fig.tight_layout()
