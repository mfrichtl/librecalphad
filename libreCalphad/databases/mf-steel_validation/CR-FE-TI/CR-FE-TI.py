...
disabled_phases = ['BCC_4SL', 'FCC_L10', 'FCC2_L10', 'LAVES2_C14', 'FCC_L12', 'HCP_L12', 'IONIC_LIQ']

phases = [phase for phase in list(db.phases.keys()) if phase not in disabled_phases]

comps = ['CR', 'FE', 'TI', 'VA']
system = '-'.join(comps[0:3])
temps = [823, 1073, 1273, 1343]
for temp in temps:
    ...
    ternary.initialize()
    ternary.do_map()
    plot_ternary(strategy=ternary, x=v.X('CR'), y=v.X('FE'), label_nodes=True, ax=ax)
    plt.title(f"{system} system, {temp} K, {date.today()}, Wang 2017")
    fig.tight_layout()
