...
def identify_variables(expr_symbols, variable_values, temp):
    ...
    for i in range(len(expr_symbols)):
        if i < temp_i:
            if isinstance(variable_values[i], np.ndarray):
                # From the minimization routine
                variable_values[i] = variable_values[i][0]
            symbol_values.append(variable_values[i])
        elif i == temp_i:
            symbol_values.append(temp)
        else:
            symbol_values.append(variable_values[i - 1])
...
def trim_conditions(
    ...
    ):  # otherwise np.delete does not seem to work as expected.
        components = np.array(components)
    for key, val in original_conditions.items():
        if str(key).startswith("X_"):
            component = str(key).split("_")[1]
            if (
                component in always_remove_list or val < solute_threshold
            ):  # trim very small quantities because they seem to cause problems.
...
def convert_conditions(conditions):
    ...
    if (
        type(list(conditions.keys())[0]) == v.StateVariable
        or type(list(conditions.keys())[0]) == v.MoleFraction
    ):  # already the correct type of variables
        return conditions
    else:
        ...
        for key, value in conditions.items():
            if key == "N":
                ...
                raise NotImplementedError(
                    f"Cannot identify pycalphad variable type for, {key} : {value}."
                )
