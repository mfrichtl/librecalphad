...
conc_arrays = {'lath-mf': {'Fe-C': np.linspace(0, 0.06, 4), 'Fe-Co': np.linspace(0, 0.33, 7), 'Fe-Cr': np.linspace(0, 0.15, 10), 'Fe-Cu': np.linspace(0, 0.08, 3),
            ...
            'epsilon-mf':  {'Fe-Mn': np.linspace(0.1, 0.235, 10),
                            ...
                            'Cr-Mn': [np.linspace(0, 0.05, 3), np.linspace(0.1, 0.2, 4)], 'Cu-Mn': [np.linspace(0, 0.025, 4), np.linspace(0.16, 0.18, 3)], 'Mn-Mo': [np.linspace(0.08, 0.18, 5), np.linspace(0, 0.04, 3)],
...
def remove_min_component(components, conditions, dependent_element='FE'):
    n = 0
    removed = False
    components = np.array(components)  # need to make sure it's cast as an ndarray for the delete function to work properly.
...
def get_components(row):
    ...
    components = ['VA']
    solutes = [comp.upper() for comp in row.keys() if len(comp) < 3 and comp != 'DG' and row[comp] > 0]
...
def get_conditions(row):
    ...
    components, conditions = trim_conditions(components, conditions, max_num_conditions=max_num_conds_DG, solute_threshold=solute_threshold, always_remove_list=remove_list, always_keep_list=do_not_remove_list)
...
def calc_aus_comp(row, db):
    ...
    """
    if pd.isnull(row['austenitization_temperature']):  # solutionizing tempeature not provided by publication
    ...
    while pd.isnull(eq_energy) and n < 10:  # arbitrary cut off for now
        if n > 0 and pdens < 3000:
            pdens += 500
        if pdens >= 3000:  # dynamically remove components with the lowest atomic fraction if calculations are still failing
            components, conditions = remove_min_component(components, conditions, dependent_element='FE')
            row['calc_error'] = True
    ...
    primary_phase = max_eq.Phase.squeeze()[~pd.isnull(max_eq.Phase.squeeze())]  # select the phase with the highest volume fraction
    if primary_phase.values != 'FCC_A1':
        print(f"Austenite not identified as primary phase for {row['conditions']}, {row['reference']}.")
        row['calc_error'] = True
        row['error_message'].append("Austenite not identified as primary phase during solutionizing treatment.")
    ...
    if len(primary_phase.values) == 0:
        ...
        row['calc_error'] = True
        row['error_message'].append(f"Cannot identify a primary phase during solutionizing treatment.")
        return row
    ...
    try:
        ...
        austenite_components, austenite_conditions = trim_conditions(austenite_components, austenite_conditions, max_num_conditions=max_num_conds_DG, solute_threshold=solute_threshold, always_remove_list=remove_list, always_keep_list=do_not_remove_list)
        ...
        row['calc_error'] = True
        row['error_message'].append(f"Error processing solution phase composition for {row['conditions']}. Message: {getattr(e, 'message', str(e))}.")
        row['austenite_composition'] = austenite_composition
        row['austenite_components'] = austenite_components
        row['austenite_conditions'] = austenite_conditions
        return row
...
def get_dG(row, db):
    ...
    while pd.isnull(energy) and n < 10:
        ...
        if pdens >= 3000:  # dynamically remove components with the lowest atomic fraction if calculations are still failing
            components, conditions = remove_min_component(components, conditions, dependent_element='FE')
            row['calc_error'] = True
            row['error_message'].append(f"Removed {n} components while calculating martensite equilibrium.")
    ...
    print(f"Phases: {phases}, DG: {energy}, PAGS: {pags_energy}, Experimental Ms: {row['martensite_start']}")
...
def calc_t0(row, db):
    ...
    calc = minimize(min_t0, 500, args=(db, components, phases, conditions), method='Nelder-Mead', bounds=temp_bounds)
    t0_temp = calc.x[0]
...
def predict_Ms(row, db, storm=False):
    ...
    if row['new_DG'] == False:  # Don't re-calculate unless the DG model changed.
        ...
        DG_threshold = 0.25  # J/mol, threshold for calling a minimization "good." Chosen semi-arbitrarily
        while (pred_DG > DG_threshold or pd.isnull(pred_DG)) and pred_Ms > 298.15 and n < 10:
            if remove_component:  # Remove the lowest concentration component until the calculation completes.
                components, conditions = remove_min_component(components, conditions, dependent_element='FE')
                row['pred_martensite_conditions'] = conditions
                row['calc_error'] = True 
                row['error_message'].append(f"Removed {n} components from martensite calculation.")        
            try:
                model_params = pd.read_json(''.join([model_param_dir, f'mf_{ms_type}_parameters.json']))
                if ms_type == 'plate':
                    if storm:
                        calc = minimize(get_model_Ms, pred_Ms, args=('plate-storm', db, components, phases, conditions, pags, pdens, [0], False), method='Nelder-Mead', bounds=temp_bounds)
                    else:
                        mf_fits = model_params['mf_plate_fits'][0]
                        calc = minimize(get_model_Ms, pred_Ms, args=('plate-mf', db, components, phases, conditions, pags, pdens, mf_fits, False), method='Nelder-Mead', bounds=temp_bounds)
                elif ms_type == 'lath':
                    if storm:
                        calc = minimize(get_model_Ms, pred_Ms, args=('lath-storm', db, components, phases, conditions, pags, pdens, [0], False), method='Nelder-Mead', bounds=temp_bounds)
                    else:
                        mf_fits = model_params['mf_lath_fits'][0]
                        calc = minimize(get_model_Ms, pred_Ms, args=('lath-mf', db, components, phases, conditions, pags, pdens, mf_fits, False), method='Nelder-Mead', bounds=temp_bounds)
                elif ms_type == 'epsilon':  # no Stormvinter epsilon model
                    mf_fits = model_params['mf_epsilon_fits'][0]
                    calc = minimize(get_model_Ms, pred_Ms, args=('epsilon-mf', db, components, phases, conditions, pags, pdens, mf_fits, False), method='Nelder-Mead', bounds=temp_bounds)
                else:
                    return np.nan
                pred_Ms = calc.x[0]
                pred_DG = calc.fun
            ...
            if pred_DG > DG_threshold or np.isnan(pred_DG):  # Unsuccesfull calculation
                ...
                if n > 0:
                    if np.abs(attempts[n] - attempts[n-1]) < 0.1:  # basically the same calculation as last time --> refining point density didn't help
                        remove_component = True
            n  += 1
        ...
        if storm:
            ...
            row['contrib_dict'], row['mf_martensite_DG'] = get_model_Ms(pred_Ms, f"{ms_type}-mf", db, components, phases, conditions, pags, pdens, mf_fits, True)
        
        row['martensite_DG'] = pred_DG
        
        
        if np.abs(pred_DG) > DG_threshold or pd.isnull(pred_DG):  # Find where the solver is performing poorly
...
def get_model_Ms(T, model_type, db, components, phases, conditions, pags, pdens, C=[0], ret_contrib_dict=False):
    conditions.update({v.T: T})
    model_energy = np.nan
    try:
...
def fit_model(orders, data, model_type):
    ...
    if model_type == 'epsilon':
        ...
        data['excess_DG'] = data.apply(lambda row: row['excess_DG'] if np.isnan(row['PAGS']) else row['excess_DG'] - (mf_epsilon_PAGS_fits[0]*np.log(row['PAGS']**mf_epsilon_PAGS_fits[1])**mf_epsilon_PAGS_fits[2] + mf_epsilon_PAGS_fits[3]), axis=1)
    ...
    for fit_term in ordered_terms:
        ...
        if model_type == 'epsilon' and fit_term != 'Fe-Mn':  # need to add manganese because it's required for epsilon to form
        ...
        assert len(sub_data['excess_DG']) > 0, f"No experimental data passed for {system_term}, {model_type} model."
    ...
    if len(quat_systems) > 0:
        raise NotImplementedError(f"No quaternary systems are implemented, but the following were passed: {quat_systems}.")
...
def get_model(components, conditions, fits, model_type, pags):
    ...
    comp_dict['FE'] = 1-np.sum([comp_dict[i] for i in list(comp_dict.keys())])  # Fe will be dependent component in these conditions
    energy_barrier = 0
    contrib_dict = {}
    ...
    for term in terms:
        ...
        if term == 'martensite_start':
            ...
            if splits[0] == 'Fe':  # Fe-X
                ...
                x_i = comp_dict[splits[0].upper()]
                x_j = comp_dict[splits[1].upper()]
                term_contrib += np.sum(np.fromiter([fit[i]*(x_i*x_j)**(i+1)/(1-x_b-x_c-x_n-x_o) for i in range(len(fit))], float))
...
def calc_model_Ms(row, db):
    ...
    while not solved and n < 5:
        if 'plate' in ms_type:
            if 'storm' in ms_type:
                DG_calc = minimize(get_model_Ms, 500, args=('plate-storm', db, components, phases, conditions, pags, pdens, new_fits), method='Nelder-Mead', bounds=temp_bounds)
            else:
                model_params = pd.read_json(''.join([model_param_dir, 'mf_plate_parameters.json']))
                new_fits = model_params['mf_plate_fits'][0]
                DG_calc = minimize(get_model_Ms, 500, args=('plate-mf', db, components, phases, conditions, pags, pdens, new_fits), method='Nelder-Mead', bounds=temp_bounds)
        elif 'lath' in ms_type:
            if 'storm' in ms_type:
                DG_calc = minimize(get_model_Ms, 500, args=('lath-storm', db, components, phases, conditions, pags, pdens, new_fits), method='Nelder-Mead', bounds=temp_bounds)
            else:
                model_params = pd.read_json(''.join([model_param_dir, 'mf_lath_parameters.json']))
                new_fits = model_params['mf_lath_fits'][0]
                DG_calc = minimize(get_model_Ms, 500, args=('lath-mf', db, components, phases, conditions, pags, pdens, new_fits), method='Nelder-Mead', bounds=temp_bounds)
        elif 'epsilon' in ms_type:  # no storm epsilon model
            model_params = pd.read_json(''.join([model_param_dir, 'mf_epsilon_parameters.json']))
            new_fits = model_params['mf_epsilon_fits'][0]
            DG_calc = minimize(get_model_Ms, 500, args=('epsilon-mf', db, components, phases, conditions, pags, pdens, new_fits), method='Nelder-Mead', bounds=temp_bounds)
        else:
            return np.nan
...
def fit_models(db, do_curve_fit=False, DG_refit=False):
    ...
    exp_data = exp_data.query("type != 'alpha' & not type.isnull() & type != 'mixed' & not DG.isnull() & predicted_type == False & ignore == False & martensite_start >= 298.15").copy()
    systems = ['Fe-']
    ...
    assert all([term in list(plate_orders.keys()) for term in plate_terms]), "MF-plate model terms do not system parameters passed."
    ...
    assert all([term in list(epsilon_orders.keys()) for term in epsilon_terms]), "MF-epsilon model terms do not match parameters passed."
    ...
    if do_curve_fit:
        ...
        model_params['baseline_temperatures'] = [np.linspace(np.min(exp_data['martensite_start']), np.max(exp_data['martensite_start']), 350)]
        ...
        for temp in model_params['baseline_temperatures'][0]:
            ...
            local_dict = {'alloy_system': 'all', 'type': 'lath-mf', 'martensite_start': [temp], 'DG': [lath_baseline], 'conditions': [{v.N: 1, v.P: 101325}], 'components': [['FE', 'VA']]}
            ...
            for system in systems:
                ...
                for x in conc_array0:
                    ...
                    for y in conc_array1:
                        ...
                        if model_type == 'epsilon-mf' or model_type == 'lath-mf':  # populate T0 temperatures for the two types that are needed
        ...
        data_to_fit.loc[:, 'martensite_start'] = data_to_fit.parallel_apply(lambda row: calc_model_Ms(row.copy(), db), axis=1)
        ...
        mf_model_data = model_data.query("type == 'lath-mf' or type == 'plate-mf' or type == 'epsilon-mf'")
...
def model_pags(db):
    ...
    exp_data = pd.read_json(''.join([exp_data_dir, 'DG_calcs_with_gpc_type.json'])).query("not PAGS.isnull() & type in @model_types").copy()
    exp_data['exp_PAGS'] = exp_data['PAGS']  # temporarily store the experimental PAGS to prevent them from being included in the calculation
...
def project_models():
    ...
    storm_model_data = pd.read_json(''.join([model_param_dir, 'stormvinter_martensite_model.json'])).fillna(0)
    t0_data = pd.read_json(''.join([model_param_dir, 't0_model.json']))
    model_data = pd.concat([mf_model_data, storm_model_data, t0_data])
    model_data = model_data.query("projected_point == False")  # drop old projected points before refitting
        ...
        for system in model_type_data['alloy_system'].unique():
            ...
            if 'Fe' in split_system:
                ...
                if len(model_system_data['DG']) <= 1:
                    ...
                    continue
                elif len(model_system_data['DG']) > 3 and order > 0:  # can't fit a 3-parameter curve to a 2-point system, which only affects some systems
                    DG_fit = curve_fit(bin_fit_func2, model_system_data[x_j], model_system_data['DG'])
                    ms_fit = curve_fit(bin_fit_func2, model_system_data[x_j], model_system_data['martensite_start'])
                else:
                    DG_fit = curve_fit(bin_fit_func1, model_system_data[x_j], model_system_data['DG'])
                    ms_fit = curve_fit(bin_fit_func1, model_system_data[x_j], model_system_data['martensite_start'])
                for x_sol in solute_arr:
                    if len(model_system_data['DG']) > 3 and order > 0:  # can't fit a 3-parameter curve to a 2-point system, which only affects some systems
                ...
                try:
                    if len(model_system_data['DG']) > 3 and order > 0:  # can't fit a 3-parameter curve to a 2-point system, which only affects some systems
                        DG_fit = curve_fit(bin_fit_func3, np.vstack([model_system_data[x_i], model_system_data[x_j]]), model_system_data['DG'])
                        ms_fit = curve_fit(bin_fit_func3, np.vstack([model_system_data[x_i], model_system_data[x_j]]), model_system_data['martensite_start'])
                    else:
                        DG_fit = curve_fit(bin_fit_func4, np.vstack([model_system_data[x_i], model_system_data[x_j]]), model_system_data['DG'])
                        ms_fit = curve_fit(bin_fit_func4, np.vstack([model_system_data[x_i], model_system_data[x_j]]), model_system_data['martensite_start'])
                    for i in x_i_arr:
                        for j in x_j_arr:
                            if len(model_system_data['DG']) > 3 and order > 0:  # can't fit a 3-parameter curve to a 2-point system, which only affects some systems
    ...
    storm_model_data = model_data.query("type == 'lath-storm' or type == 'plate-storm'")
    model_data = model_data.query("type == 'lath-mf' or type == 'plate-mf' or type == 'epsilon-mf'")  # drop the storm model info
    model_data = model_data.reset_index(drop=True)
...
def make_plots():
    ...
    exp_data = pd.read_json(''.join([model_param_dir, 'martensite_experimental_model_data.json'])).query("ignore == False & predicted_type == False").sort_values(by='type')
        ...
        if 'Fe' in split_term:  # only Fe-X binary systems, can include all models on one plot
            ...
            for martensite_type in ['lath-plate', 'epsilon']:
                if martensite_type == 'epsilon':
                    ...
                    model_Ms_query = ['lath-mf', 'plate-mf', 'T0-alpha', 'lath-storm', 'plate-storm']
                    label_var = '\\alpha'
                model_exp_data = system_exp_data.query("type in @exp_type_query")
                if len(model_exp_data) == 0:  # system not modeled for this model type if this evaluates to true
                    continue
                ...
                fig, ax = plt.subplots(figsize=figsize)
                sns.scatterplot(data=model_exp_data, x=x, y='martensite_start', hue='type', ax=ax, style='reference', s=50)
            ...
            for model in models:
                ...
                # make contour plots
                crange = [np.min(np.concatenate([system_model_data['DG'], model_exp_data['DG']])), np.max(np.concatenate([system_model_data['DG'], model_exp_data['DG']]))]
...
def load_exp_data():
    ...
    elements = ['Al', 'B', 'C', 'Ce', 'Co', 'Cr', 'Cu', 'Fe', 'Ge', 'H', 'Hf', 'Mn', 'Mo', 'N', 'Nb', 'Ni', 'O', 'P', 'Pb', 'S', 'Si', 'Ta', 'Ti', 'V', 'W', 'Zr']
    ...
    drop_cols = ['LCL', 'UCL', 'comment', 'comments', 'composition', 'figure_no', 'heating_rate', 'material_at%', 'material_wt%', 'method', 'system', 'verified']
...
def calc_DG(db, DG_refit):
    ...
    for reference in data['reference'].unique():
        ...
        sub_df['non_FCC_solution_phases'] = sub_df.apply(lambda row: np.sum([phase != 'FCC_A1' and phase != '' for phase in row['solutionizing_phases']]), axis=1)
    ...
    if DG_refit:
        ...
        old_data = old_data.drop([col for col in old_data.columns if col not in data.columns], axis=1)  # drop other columns that are added in subsequent calculations
    ...
    data.loc[:, 'DG_version'] = data.apply(lambda row: time.time() if row['new_DG'] == True else row['DG_version'], axis=1)  # update DG version if different calculated DG
...
def do_martensite_start(db, DG_refit):
    ...
    for model_type in types:
        model_params[model_type] = pd.read_json(''.join([model_param_dir, f'mf_{model_type}_parameters.json']))
    ...
    exp_data = pd.read_json(''.join([exp_data_dir, 'DG_calcs_with_gpc_type.json']))
    exp_data = exp_data.query("type in @types & martensite_start >= 298.15 & not reference.isnull() & ignore == False & not DG.isnull()").reset_index(drop=True)
    ...
    if DG_refit:  # refit the Stormvinter predictions
        ...
        storm_exp = storm_exp.query("type in @types & martensite_start >= 298.15 & not reference.isnull() & ignore == False & not DG.isnull()").reset_index(drop=True)
        temp_df = pd.DataFrame()
...
def make_parity_plots():
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=exp_data, x='martensite_start', y='mf_martensite_start', hue='Type Prediction',  style='Type', s=50, ax=ax)
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=exp_data.query("`Type Prediction` == 'Known Type'"), x='martensite_start', y='mf_martensite_start', hue='alloy_system', s=50, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals+mf_combined_known_type_RMSE, color='black', linestyle='--', ax=ax)
    sns.lineplot(x=line_vals, y=line_vals-mf_combined_known_type_RMSE, color='black', linestyle='--', ax=ax)
    ax.set_xlabel(r'Experimental $M_\mathrm{s}$ (K)')
    ax.set_ylabel(r'Predicted $M_\mathrm{s}$ (K)')
    ax.text(400, 1050, "Alloys Predicted: " + str(len(exp_data.query("`Type Prediction` == 'Known Type'"))))
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=lath_plate_exp, x='martensite_start', y='mf_martensite_start', hue='Type Prediction', style='Type', s=50, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals, ax=ax)
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=lath_plate_exp.query("`Type Prediction` == 'Known'"), x='martensite_start', y='mf_martensite_start', hue='alloy_system', s=50, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals+mf_lp_RMSE, color='black', linestyle='--', ax=ax)
    sns.lineplot(x=line_vals, y=line_vals-mf_lp_RMSE, color='black', linestyle='--', ax=ax)
    ax.set_xlabel(r'Experimental $M_\mathrm{s}$ (K)')
    ax.set_ylabel(r'Predicted $M_\mathrm{s}$ (K)')
    ax.text(400, 1050, f"Alloys Predicted: " + str(len(lath_plate_exp.query("`Type Prediction` == 'Known'"))))
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=epsilon_exp, x='martensite_start', y='mf_martensite_start', hue='Type Prediction', s=50, ax=ax)
    sns.lineplot(x=line_vals, y=line_vals, ax=ax)
    ...
    ax.set_xlabel(r'Experimental $M_\mathrm{s}$ (K)')
    ax.set_ylabel(r'Predicted $M_\mathrm{s}$ (K)')
    ax.text(350, 460, f"Alloys Predicted: " + str(len(epsilon_exp.query("`Type Prediction` == 'Known'"))))
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=exp_data.tail(25), x='martensite_start', y='error', hue='Type', style='alloy_system', s=50, ax=ax)
    ax.set_xlabel(r'Experimental $M_\mathrm{s}$ (K)')
    ax.set_ylabel(r'Predicted $M_\mathrm{s}$ Error (K)')
    fig.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.scatterplot(data=exp_data.query("`Type Prediction` == 'Known'").tail(25), x='martensite_start', y='error', hue='Type', style='alloy_system', s=50, ax=ax)
    ax.set_xlabel(r'Experimental $M_\mathrm{s}$ (K)')
    ax.set_ylabel(r'Predicted $M_\mathrm{s}$ Error (K)')
    fig.tight_layout()
